...
//小七修改
function qiehuantp($gg_id)
{
$sql = "SELECT * FROM `ecs_ad` WHERE position_id =$gg_id ORDER BY `ad_id` ASC LIMIT 0 , 5";
     $res = $GLOBALS['db']->query($sql);
    $list = array();
  while ($row = $GLOBALS['db']->fetchRow($res))
    {
		$arr['ad_name'] =$row['ad_name'];
		$arr['ad_code'] = 'data/afficheimg/'.$row['ad_code'];	
		$arr['ad_link'] = $row['ad_link'];	
        $list[] = $arr;
    }
    return $list;
}
function ggtp($gg_id,$aid)
{

$sql = "SELECT * FROM `ecs_ad` WHERE position_id =$gg_id and ad_id =$aid ORDER BY `ad_id` ASC LIMIT 0 , 1";
...
function get_comments($kaishi,$num)
...

{
   ...
   $sql = 'SELECT a.*,b.goods_id,b.goods_thumb,b.goods_name FROM '. $GLOBALS['ecs']->table('comment') .
            ...
            ' ORDER BY a.add_time DESC LIMIT ' . $kaishi .','. $num;    
...
function get_brandsyi($cat = 0, $app = 'brand')

{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            ...
            "WHERE b.brand_logo !='' and g.brand_id = b.brand_id $children AND is_show = 1 " .
...
//小七修改
function tjiansp($fenlei)
{ 
$sql = "SELECT * FROM  ecs_goods WHERE is_hot = 1 and is_delete = 0 and cat_id = $fenlei ORDER BY `add_time` DESC LIMIT 0 , 10";
     $res = $GLOBALS['db']->query($sql);
    $list = array();
  while ($row = $GLOBALS['db']->fetchRow($res))
    {
		if ($row['promote_price'] > 0)
        {
          ...
          $arr['promote_price'] = $promote_price > 0 ? price_format($promote_price) : '';
        }
        else
        {
          $arr['promote_price'] = '';
        }
		$arr['goods_img'] = $row['goods_thumb'];
		$arr['goods_name'] = $row['goods_name'];
		$arr['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$arr['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
		$arr['short_style_name'] = add_style($arr['short_name'], $row['goods_name_style']);
		$arr['market_price'] = price_format($row['market_price']);
		$arr['id'] = $row['goods_id'];
